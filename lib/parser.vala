...
// 		  
		...
		public void open_scope(Scanner scanner, Scope? expected_scope, int line_ix, string line, int length, Marker m,	   ArrayList<Scope> all_scopes, ArrayList<Scope> closed_scopes, ArrayList<Scope> removed_scopes ) {
		...
		public void single_scope(Scanner scanner, Scope? expected_scope, int line_ix, string line, int length, Marker m, ArrayList<Scope> all_scopes, ArrayList<Scope> closed_scopes, ArrayList<Scope> removed_scopes) {
			var s = new Scope(this.buffer, m.pattern.name);
			s.pattern = m.pattern;
			s.open_match = m.match;
			set_start_mark_safely(s, m, line_ix, length, 0);
...
//				stdout.printf("src: '%s'\n", src.str);
		...
		public void set_inner_start_mark_safely(Scope scope, Marker m, int line_ix, int length, int cap) {
			...
			if (to == length && this.buffer.get_line_count() > line_ix+1) 
				...
				scope.inner_start_mark_set(line_ix, int.min(to, length-1), true);
		}

		public void set_inner_end_mark_safely(Scope scope, Marker m, int line_ix, int length, int cap) {
			int to = m.match.begin(cap);
			if (to == length && this.buffer.get_line_count() > line_ix+1) {
   			scope.inner_end_mark_set(line_ix, length-1, true);
			}
			else {
		...
		public void set_end_mark_safely(Scope scope, Marker m, int line_ix, int length, int cap) {
			int to = m.match.end(cap);
			if (to == length && this.buffer.get_line_count() > line_ix+1) {
				scope.end_mark_set(line_ix, length-1, true);
			}
			else {
			...
			if (captures != null) {
				foreach (int cap in captures.get_keys()) {
					if (m.match.begin(cap) != -1) {
						...
						set_end_mark_safely(s, m, line_ix, length, cap);
						s.is_open = false;
						s.is_capture = true;
						s.parent = scope;
